function main()
{
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            var i;
            var temp = sessionStorage.getItem("searchValue");
            for(i = 1;i < reponse.length; i++) {     //pour chaque annonce presente dans la reponse du serveur
                if ((temp== null)||((FindPostTag('_titre_annonce":"',reponse[i]).toUpperCase()).includes(temp.toUpperCase()))){                //Creation de l'espace entre chaque annonce
